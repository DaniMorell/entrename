var path = require('path');

// Postgres DATABASE_URL = postgres://user:passwd@host:port/database
// SQLite   DATABASE_URL = sqlite://:@:/
var url = process.env.DATABASE_URL.match(/(.*)\:\/\/(.*?)\:(.*)@(.*)\:(.*)\/(.*)/);
var DB_name  = (url[6]||null);
var user     = (url[2]||null);
var pwd      = (url[3]||null);
var protocol = (url[1]||null);
var dialect  = (url[1]||null);
var port     = (url[5]||null);
var host     = (url[4]||null);
var storage  = process.env.DATABASE_STORAGE;

// Cargar Modelo ORM
var Sequelize = require('sequelize');

// Usar BBDD SQLite o Postgres
var sequelize = new Sequelize(DB_name, user, pwd, 
  { dialect:  protocol,
    protocol: protocol,
    port:     port,
    host:     host,
    storage:  storage,  // solo SQLite (.env)
    omitNull: true      // solo Postgres
  }      
);

//var sequelize = new Sequelize(null, null, null, {dialect: "sqlite", storage:"entrenes.sqlite"});

// Importar definicion de la tabla Entrene
var entrene_path = path.join(__dirname,'entrenes');
var Entrene = sequelize.import(entrene_path);

// Importar definicion de la tabla Comment
/*var comment_path = path.join(__dirname,'comment');
var Comment = sequelize.import(comment_path);

Comment.belongsTo(Entrene);
Entrene.hasMany(Comment);*/

// exportar tablas
exports.Entrene = Entrene;
//exports.Comment = Comment; 

// sequelize.sync() inicializa tabla de preguntas en DB
sequelize.sync().then(function() {
	Entrene.count().then(function (count){
	  if(count === 0) {   // la tabla se inicializa solo si está vacía
		Entrene.create({entrenamiento: 'Entrenamiento Nº1'})
		.then(function(){console.log('Base de datos (tabla entrenes) inicializada')});
	  };
	});
 });